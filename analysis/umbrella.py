...
# ---------------- Sektor Onerisi Sihirbazi (soru-cevapla hedef sektor belirleme) ----------------

WIZARD_VADE_OPTIONS: list[str] = ["Kısa vadeli (1 yıldan az)", "Orta vadeli (1-3 yıl)", "Uzun vadeli (3+ yıl)"]
WIZARD_RISK_OPTIONS: list[str] = ["Düşük", "Orta", "Yüksek"]
WIZARD_ONCELIK_OPTIONS: list[str] = ["İstikrar", "Dengeli", "Büyüme"]

# Her sektor/tema icin KABACA bir risk/oynaklik etiketi (heuristik, kesin bir siniflandirma degildir).
# Sihirbaz, kullanicinin risk tolerransi+vade+oncelik cevaplarindan hesapladigi "hedef risk seviyesi"ne
# en yakin profildeki secili sektorlere daha fazla agirlik vermek icin kullanir.
SECTOR_RISK_PROFILE: dict[str, str] = {
    "Genel / Karma": "Orta",
    "Amerika Hisse Senedi": "Yüksek",
    "Avrupa Hisse Senedi": "Yüksek",
    "Asya Hisse Senedi": "Yüksek",
    "Çin Hisse Senedi": "Yüksek",
    "Hindistan Hisse Senedi": "Yüksek",
    "Japonya Hisse Senedi": "Yüksek",
    "Küresel / Yabancı": "Yüksek",
    "Bankacılık / Finans": "Orta",
    "Teknoloji": "Yüksek",
    "Sanayi": "Orta",
    "Enerji": "Orta",
    "Sağlık": "Orta",
    "Gayrimenkul / İnşaat": "Orta",
    "Tarım / Gıda": "Düşük",
    "Savunma Sanayii": "Orta",
    "Temettü Odaklı": "Düşük",
    "Sürdürülebilirlik / ESG": "Orta",
    "Kıymetli Maden": "Düşük",
    "Endeks (BIST)": "Orta",
    "Girişim Sermayesi": "Yüksek",
    "Katılım / Faizsiz": "Düşük",
    "Ulaştırma": "Orta",
    "Telekom": "Düşük",
    "Perakende / Gıda": "Düşük",
    "Holding": "Orta",
    "Yenilenebilir Enerji": "Yüksek",
}

_RISK_SCORE: dict[str, float] = {"Düşük": 1.0, "Orta": 2.0, "Yüksek": 3.0}
_VADE_ADJ: dict[str, float] = {
    "Kısa vadeli (1 yıldan az)": -0.4, "Orta vadeli (1-3 yıl)": 0.0, "Uzun vadeli (3+ yıl)": 0.4,
}
_ONCELIK_ADJ: dict[str, float] = {"İstikrar": -0.4, "Dengeli": 0.0, "Büyüme": 0.4}


def wizard_recommend_targets(
    selected_sectors: list[str],
    risk_toleransi: str,
    vade: str,
    oncelik: str,
) -> tuple[list[tuple[str, float]], str]:
    """Soru-cevap sihirbazindan hedef sektor/tema yuzdelerini hesaplar.

    Secilen sektorlerin HEPSI sonuca dahil edilir (kullanicinin ilgi alanini disleyerek "daha
    isabetli" gorunmeye calismiyoruz); agirliklar sektorun risk profilinin, kullanicinin risk
    toleransi + yatirim vadesi + onceliginden hesaplanan "hedef risk seviyesi"ne yakinligina gore
    dagitilir - hedefe daha yakin sektorler orantili olarak daha fazla pay alir.
    """
    if not selected_sectors:
        return [], "Hiç sektör/tema seçilmedi."

    hedef_risk = _RISK_SCORE.get(risk_toleransi, 2.0) + _VADE_ADJ.get(vade, 0.0) + _ONCELIK_ADJ.get(oncelik, 0.0)
    hedef_risk = max(1.0, min(3.0, hedef_risk))

    weights: dict[str, float] = {}
    for s in dict.fromkeys(selected_sectors):
        sektor_risk = _RISK_SCORE.get(SECTOR_RISK_PROFILE.get(s, "Orta"), 2.0)
        weights[s] = 1.0 / (1.0 + abs(sektor_risk - hedef_risk))

    total = sum(weights.values())
    targets = [(s, round(w / total * 100.0, 1)) for s, w in weights.items()]
    fark = round(100.0 - sum(p for _, p in targets), 1)
    if targets and abs(fark) >= 0.1:
        idx = max(range(len(targets)), key=lambda i: targets[i][1])
        s, p = targets[idx]
        targets[idx] = (s, round(p + fark, 1))
    targets.sort(key=lambda x: x[1], reverse=True)

    risk_etiketleri = ", ".join(f"{s} ({SECTOR_RISK_PROFILE.get(s, 'Orta')} risk)" for s, _ in targets)
    gerekce = (
        f"Risk toleransınız (**{risk_toleransi}**), yatırım vadeniz (**{vade}**) ve önceliğiniz "
        f"(**{oncelik}**) birleştirilerek 1.0 (düşük risk) - 3.0 (yüksek risk) ölçeğinde yaklaşık "
        f"**{hedef_risk:.1f}** puanlık bir hedef risk seviyesi hesaplandı. Seçtiğiniz her sektöre pay "
        f"verildi; risk profili bu hedefe daha yakın olan sektörler orantılı olarak daha fazla pay "
        f"aldı: {risk_etiketleri}. Bu tamamen kural tabanlı bir başlangıç önerisidir — aşağıdaki "
        "tablodan dilediğiniz gibi değiştirebilirsiniz."
    )
    return targets, gerekce
